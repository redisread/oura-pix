...
feedback.post("/", zValidator("json", submitSchema), async (c) => {
  ...
  try {
    ...
    const gen = await db
      ...
      .limit(1);
    if (gen.length === 0) {
      return c.json({ success: false, error: { code: "NOT_FOUND", message: apiMessage(c, "generationNotFound") } }, 404);
    }
...
feedback.get("/", async (c) => {
  ...
  if (!generationId) return c.json({ success: false, error: { code: "BAD_REQUEST", message: apiMessage(c, "badRequest") } }, 400);
